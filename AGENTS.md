...
- **pydantic-settings** — в зависимостях pyproject.toml, но в коде не используется
...
src/pdns_mcp_server/
  ...
  rrset.py     — RRset (dataclass): нормализация name/type, формирование PATCH-тела
...
tests/
  ...
  test_rrset.py  — 7 тестов (RRset.to_dict, нормализация)
